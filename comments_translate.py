...
def translate_content(text, model="gpt-3.5-turbo", max_tokens=500):
    ...
    user_message=f"""
Please reply to the text comments , but no c code, provided below, from English to Japanese, keep the formatting:
{text}
"""
    ...
    with open(en_file, "w", encoding="utf-8") as file:
        for nl_comment in nl_comments:
            if nl_comment:
                ...
                print(f"Translated comment: {nl_comment} to {translated_comment}")

def process_directory(directory):
    for filename in os.listdir(directory):
        if filename.endswith(".c.nl.txt"):
            nl_file = os.path.join(directory, filename)
            en_file = os.path.join(directory, filename.replace(".en.txt", ".jp.txt"))
